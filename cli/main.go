...
func cmdInstall(d *device.Device, what, catalogURL string) {
	defer d.Close()
	// npm's rule: a bare name is always a catalog id; installing a local
	// folder requires an explicit path (./myapp), and it must be an app.
	if strings.ContainsAny(what, "/\\") {
		st, err := os.Stat(what)
		if err != nil || !st.IsDir() {
			die("%s is not a folder", what)
		}
		if _, err := os.Stat(filepath.Join(what, "external.manifest.json")); err != nil {
			die("%s has no external.manifest.json — not an app folder", what)
		}
